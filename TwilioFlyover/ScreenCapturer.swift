...
//  TwilioFlyover
//
//  Created by Lizzie Siegle on 10/10/18.
//  Copyright © 2018 Lizzie Siegle. All rights reserved.
//

import TwilioVideo

class ScreenCapturer: NSObject, TVIVideoCapturer {
    ...
    func startTimer() {
        ...
        displayTimer = CADisplayLink(target: self, selector: #selector(ScreenCapturer.captureView))
    ...
    func registerNotificationObservers() {
        ...
        willEnterForegroundObserver = notificationCenter.addObserver(forName: NSNotification.Name.NSExtensionHostWillEnterForeground, object: nil, queue: OperationQueue.main, using: { (Notification) in self.displayTimer?.isPaused = false;
            })
        
        didEnterBackgroundObserver = notificationCenter.addObserver(forName: NSNotification.Name.NSExtensionHostDidEnterBackground,
                                                                    object: nil,
